  ...
  return (
    <div className="flex flex-col gap-5 w-full bg-foreground border border-border-subtle p-4 sm:p-5 rounded-2xl shadow-sm">
      {data?.items?.map((item, index) => {
        ...
        return (
          <div key={item.id} className="flex flex-col gap-4 w-full bg-background border border-border-subtle p-4 rounded-xl">
            {/* Question */}
            <div className="flex items-start gap-3">
              ...
              <p className="font-semibold leading-relaxed">
                {item.question.en ??
                  Object.values(item.question).find(Boolean)}
              </p>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-medium leading-normal">
                      {option.text.en ??
                        Object.values(option.text).find(Boolean)}
                    </span>
                  </button>
                );
              })}
