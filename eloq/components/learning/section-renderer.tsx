...
{lesson.blocks.map((block, index) => {
  ...
  return (
    <article key={block.id} className="w-full flex flex-col gap-3 pb-6 border-b border-neutral-200/10 last:border-0">
      <header className="flex items-start gap-3 w-full">
        ...
        <div className="flex items-start gap-2 text-white sm:text-lg font-bold leading-normal text-foreground">
          <p>{instruction?.text.en || instruction?.en || "......"}</p>

          {block.extensions?.audio?.url ? (
            <button type="button" aria-label="Play audio" className="mt-1 shrink-0 text-eloq-purple transition hover:scale-105">
              <Volume2 size={19} />
            </button>
          ) : (
            <button
              ...
              onClick={() => alert("Audio support will be available soon.")}
